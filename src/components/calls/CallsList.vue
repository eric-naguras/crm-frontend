...
<script>
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      onMounted(async () => {
        console.log('Components CallsList: onMounted')
        loading.value = true
        await store.calls.methods.getCalls(props.id)
        loading.value = false
        console.log('Components CallsList: got calls, result:', store.calls.state.calls)
      })
      ...
      const rows = computed({
        get() {
          let result = []
          if (props.tab === 'calls') {
            result = store.calls.state.calls
          }
          // if (props.tab === 'venues') {
      ...
      const rowClicked = (evt, row, index) => {
        console.log('Component CallsList: Row clicked', evt, row, index)
      }
